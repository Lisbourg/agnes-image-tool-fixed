...
# ==================== 辅助函数 ====================
def get_video_dimensions(resolution_preset, ratio, custom_width, custom_height):
    """根据用户输入计算最终宽高：优先自定义值，否则用预设档位+比例"""
    if custom_width and custom_height and custom_width > 0 and custom_height > 0:
        return int(custom_width), int(custom_height)
    preset = VIDEO_RESOLUTION_PRESETS.get(resolution_preset, {})
    if ratio in preset:
        size = preset[ratio]
        return size["width"], size["height"]
    fallback = RATIO_TO_SIZE.get(ratio, {"width": 1152, "height": 768})
    ...
    with gr.Blocks(title="Agnes Creator Studio") as demo:
        ...
        def generate_text2video(
            ...
            try:
                ...
                # 计算最终宽高（使用新增的辅助函数）
                w, h = get_video_dimensions(resolution_preset, ratio, width, height)
        ...
        def generate_image2video(
            api_key, base_url, model,
            image_path, image_url_text, prompt, negative_prompt,
            resolution_preset, ratio, width, height,
            duration, frame_rate, num_frames, seed, steps
        ):
            """图生视频处理 - 修正版，支持分辨率预设并修复内部调用错误"""
            if not api_key:
                return None, "❌ 请先配置 API Key"
            if not prompt.strip():
                return None, "❌ 请输入视频描述"
            ...
            try:
                ...
                if image_url_text and image_url_text.strip():
                    ...
                    video_url = gen.image_to_video_url(
                        image_url=image_url_text.strip(),
                        prompt=prompt,
                        duration=int(duration),
                        aspect_ratio=ratio,
                        frame_rate=int(frame_rate),
                    ...
                    print(f" 图生视频: 使用本地图片={image_path}")
                    video_url = gen.image_to_video(
                        image_path=image_path,
                        prompt=prompt,
                        duration=int(duration),
                        aspect_ratio=ratio,
